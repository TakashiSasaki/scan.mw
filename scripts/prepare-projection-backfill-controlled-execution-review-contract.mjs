...
import { buildProjectionBackfillControlledExecutionReviewContract, formatProjectionBackfillControlledExecutionReviewContract } from './lib/projection-backfill-controlled-execution-review-contract.mjs';
...
async function main() {
  ...
  if (!args.packet) {
    if (isJson) {
       console.error(JSON.stringify({ error: "--packet is required." }));
    } else {
       console.error("Error: --packet is required.");
    }
    process.exit(1);
  }
  ...
  let packetData;
  try {
    const raw = fs.readFileSync(args.packet, 'utf-8');
    packetData = JSON.parse(raw);
  } catch (err) {
    if (isJson) {
       console.error(JSON.stringify({ error: `Could not read or parse packet manifest at ${args.packet}: ${err.message}` }));
    } else {
       console.error(`Error: Could not read or parse packet manifest at ${args.packet}: ${err.message}`);
    }
    process.exit(1);
  }
  ...
  const input = {
    controlledExecutionDesignPacket: packetData,
  ...
  const contract = buildProjectionBackfillControlledExecutionReviewContract(input);

  if (isJson) {
    console.log(formatProjectionBackfillControlledExecutionReviewContract(contract, { json: true }));
  } else {
    console.log(formatProjectionBackfillControlledExecutionReviewContract(contract));
  }
